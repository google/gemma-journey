...
namespace GoogleDeepMind.GemmaSampleGame
{
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField] private float stopTurnDegree = 0.1f;
        ...
        private readonly int turnHash = Animator.StringToHash("Side");
        ...
        /// <summary>
        /// Receive interact event to turn the player toward NPC
        /// </summary>
        /// <param name="target">Interactable that this character is interacting with</param>
        public void HandleOnInteract(InteractableObject target)
        {
            TurnOvertime(target.transform, this.GetCancellationTokenOnDestroy()).Forget();
        }
        ...
        public async UniTask TurnOvertime(Transform target, CancellationToken cancellationToken)
        {
            ...
            if (angle < 0)
            {
                _animator.SetInteger(turnHash, -1);
            }
            else
            {
                _animator.SetInteger(turnHash, 1);
            ...
            while (true)
            {
                ...
                if (target != null)
                {
                    ...
                    if (angleDifference < stopTurnDegree)
                    {
                        break;
                    }
                }
            ...
            _animator.SetInteger(turnHash, 0);
        }
    }
}