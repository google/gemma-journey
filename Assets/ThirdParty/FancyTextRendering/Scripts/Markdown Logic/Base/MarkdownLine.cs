...
namespace LogicUI.FancyTextRendering.MarkdownLogic
{
    internal class MarkdownLine
    {
        ...
        {
            if (!spacingFontUnits.IsCloseTo(0))
                Builder.Prepend("<size=" + spacingFontUnits.ToString(CultureInfo.InvariantCulture) + "em>" + ZeroWidthSpace + "</size>\n");
        }
        public void AddVerticalWhitespaceAfter(float spacingFontUnits)
        {
            if (!spacingFontUnits.IsCloseTo(0))
                Builder.Append("\n<size=" + spacingFontUnits.ToString(CultureInfo.InvariantCulture) + "em>" + ZeroWidthSpace + "</size>");
        }
        ...
        public string Finish()
        {
            ...
            void ReplaceEscapes()
            {
                ...
                {
                    int index = Builder.IndexOf(EscapeCharacter, countingIndex);
                    ...
                    // Skip one character. This allows you to escape escapes, i.e. '\\' becomes '\'. Big brain moment
        ...
        int CountPrecedingEscapesCharacters(int index)
        {
            ...
            {
                if (Builder[index - 1] == EscapeCharacter)
                    count++;
                else
                    break;
            }
